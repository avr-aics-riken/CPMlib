...
int main( int argc, char **argv )
{
  ...
  {
    const double *org = domainInfo[i]->GetOrigin();
    const double *rgn = domainInfo[i]->GetRegion();
    const int    *vox = domainInfo[i]->GetVoxNum();
    const int    *div = domainInfo[i]->GetDivNum();
    const int    *nod = domainInfo[i]->GetNodNum();
    if( i==0 )
    {
      // プロセスグループ0で領域分割
      //if( (ret = paraMngr->VoxelInit( domainInfo[i], 3, 6 )) != CPM_SUCCESS )
      if( (ret = paraMngr->NodeInit( (int *)div, (int *)nod, (double *)org, (double *)rgn, 3
                                       , 6, DIV_COMM_SIZE )) != CPM_SUCCESS )
      {
        cerr << "NodeInit error : " << ret << endl;
        return ret;
      }
//#ifdef _DEBUG
...
//#endif
    ...
    else
    {
      ...
      if( procGrpNo >= 0 )
      {
        ...
        if( (ret = paraMngr->NodeInit( (int *)div, (int *)nod, (double *)org, (double *)rgn, 3
                                        , 3, DIV_COMM_SIZE, procGrpNo )) != CPM_SUCCESS )
        {
          cerr << "NodeInit error : " << ret << endl;
          return ret;
        }
